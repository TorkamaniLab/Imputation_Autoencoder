...
from DSAE_TORCH_ARG_PHASED import Autoencoder
...
def get_predicted_gen_known_indices(new_df_sample, j):
    #0101
    if new_df_sample[j + 3] == 1 and new_df_sample[j + 1] == 1:
        return "1/1:2:0:0:0:1"
    #1010
    elif new_df_sample[j] == 1 and new_df_sample[j + 2] == 1:
        return "0/0:0:1:0:0:0"
    #1001
    elif new_df_sample[j] == 1 and new_df_sample[j + 3] == 1:
        return "0/1:1:0:1:0:0"
    #0110
    elif new_df_sample[j + 1] == 1 and new_df_sample[j + 2] == 1:
...
def get_predicted_gen_imputed(y_pred_sample, j, is_debug_on):
    ...
    #1010
    Pa = y_pred_sample[j]*y_pred_sample[j + 2]
    #1001
    Pab = y_pred_sample[j]*y_pred_sample[j + 3]
    #0110
    Pba = y_pred_sample[j + 1]*y_pred_sample[j + 2]
    #0101
    Pb = y_pred_sample[j + 1]*y_pred_sample[j + 3]
    ...
    #MINIMAC method: https://genome.sph.umich.edu/wiki/Minimac3_Info_File#Dosage
    #Df1 = np.sum([Pab, Pba, 2*Pb])
    #Df1 = np.round(Df1, 4)
    #easier way, just sum alt probabilities
    Df1 = y_pred_sample[j + 1]+y_pred_sample[j + 3]
    ...
    if D == 3:
        ...
        return None
    gen += str(Df1) + ":" + str(Pa) + ":" + str(Pab) + ":" + str(Pba) + ":" + str(Pb)
    return gen
...
def export_vcf(posfile, pred_out, infile, output_file):
    ...
    comments = \
        ...
        "##FORMAT=<ID=Paa,Number=1,Type=Float,Description=\"Imputation probability for homozigous reference : Pa=(1-y_pred[i][j])*(1-y_pred[i][j+1])\">\n" + \
        "##FORMAT=<ID=Pab,Number=1,Type=Float,Description=\"Imputation probability for heterozigous : Pab=y_pred[i][j]\">\n" + \
        "##FORMAT=<ID=Pba,Number=1,Type=Float,Description=\"Imputation probability for heterozigous : Pab=y_pred[i][j+1]\">\n" + \
        "##FORMAT=<ID=Pbb,Number=1,Type=Float,Description=\"Imputation probability for homozigous alternative : Pb=y_pred[i][j]*y_pred[i][j+1]\">\n" + \
        my_header
    ...
    columns = ['.', '.', '.', 'GT:DS:Paa:Pab:Pba:Pbb']*len(refpos)
...
def main(argv):
    ...
    reconstructed = torch.reshape(reconstructed, [-1,int(len(new_df[0])/2),2])
    reconstructed = F.softmax(reconstructed, dim=-1)
    reconstructed = torch.flatten(reconstructed, start_dim=1)
